...
| D3 | **Run the funnel and oracle on HI-Medium** | **HALF DONE 2026-09-05.** Funnel run. **Oracle is BLOCKED ON MEMORY, measured not guessed** -- see D3b. | part |
...
| D3b | **Make `eval_oracle.collect_pool` retain feature vectors, not `Candidate` objects** | The oracle keeps the whole Candidate for every candidate of every cycle, in two pools, neither freed before the other is built. Measured: ~1,064 bytes PICKLED per candidate (live objects run 3-5x that), and HI-Medium yields ~3.36M candidates per pool. That is 10-17 GB per pool on a **16.5 GB** machine. It needs only `vectorise(c.features)`, the ring label, `t` and `size` -- roughly a 10x reduction, and numerically identical, so equivalence is verifiable by re-running HI-Small's oracle and diffing the JSON. | ~3h incl. verification |
