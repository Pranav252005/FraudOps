...
| D5 | **`ring_recall@k` intervals are invalid** | The point estimate falls OUTSIDE its own CI -- HI-Medium at all three k, HI-Small at k=20 and k=50. Cluster bootstrap on a UNION statistic: a resample covers ~63% of distinct cycles, so the union of rings found is systematically smaller than over the full set and the interval is biased low by construction. `ring_recall` is quoted in headline tables. p@k is unaffected (ratio of sums, not a union). | ~2h |
| D6 | **`eval_funnel.py` misreports where it wrote** | Prints `data/funnel.json` while writing `funnel-HI-Medium.*`. Same defect fixed in eval_phase2.py and not swept. | ~15m |
