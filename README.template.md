...
`n = {{count:n_held_out_cycles}}` held-out cycles. Every interval in the
...
draws per configuration rather than {{count:n_held_out_cycles}} correlated
...
{{count:n_identity_configs_passing}} of {{count:n_identity_configs}}
configurations pass the kill rule against a pre-registered floor of
{{count:n_identity_configs_required}}. Offices of 20–200 people on one IP
...
{{ci_signed:identity_coverage_delta_rotation}}**, paired on the same worlds —
...
primary configuration: {{metric:identity_seed_reach_coverage}} against
{{metric:amlworld_seed_reach_coverage}} on the same definition. It was demoted
...
The repository's `n` is {{count:n_held_out_cycles}} held-out cycles and every
...
{{count:n_identity_configs}} configurations pass**, against a pre-registered
floor of {{count:n_identity_configs_required}}. Failures are marked `TOO_EASY`
and excluded from every later phase rather than averaged in.

### The features cannot read the generator

{{count:n_identity_features}} features in six families, of which
{{count:n_identity_features_excluded}} are excluded — every attribute whose
...
bit-identical, and
{{count:n_identity_features_tripping_leak_auc}} of the remaining features reach
the pre-registered leak threshold over
{{count:n_identity_candidates_scored}} scored candidates.
...
|---|---:|---|
| synthetic-identity-v1 | **{{metric:identity_seed_reach_coverage}}** | {{ci:identity_seed_reach_coverage}} |
| amlworld-hi-small | {{metric:amlworld_seed_reach_coverage}} | {{ci:amlworld_seed_reach_coverage}} |
...
everything else fixed:
    ...
    coverage(rotation 0.7) - coverage(rotation 0.3)
      = {{signed:identity_coverage_delta_rotation}} {{ci_signed:identity_coverage_delta_rotation}}
