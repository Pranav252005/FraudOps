...
- [What else was measured on AMLworld](#what-else-was-measured-on-amlworld)
...
It had to survive a kill rule committed before the generator existed, naming
four trivial baselines any one of which solving the problem would have thrown
the domain out: degree, attribute multiplicity, component size, and
rare-attribute multiplicity. **{{count:n_identity_configs_passing}} of
{{count:n_identity_configs}} configurations pass**, against a pre-registered
floor of {{count:n_identity_configs_required}}; the rest are marked `TOO_EASY`
...
second implementation of any of them. Three things deliberately did not
transfer, and each is argued where it is decided: no window, an exogenous seed
rule, and the Jaccard floor inherited unchanged so the two stay comparable.

**The pre-registered dose-response held, on all three sub-predictions including
the mechanism. The cross-domain prediction that motivated the domain was
refuted** — identity fragments *less* than AMLworld, not more. It had been
demoted from a test to a descriptive observation before measurement, which is
the only reason it is a reportable result rather than a failed headline. The two
domains are reported side by side and never pooled; `require_same_dataset`
refuses.

The full account lives in its own documents rather than in this file:

| | |
|---|---|
| why this domain, and the kill rule | [`docs/PHASEA-IDENTITY-BACKGROUND.md`](docs/PHASEA-IDENTITY-BACKGROUND.md) |
| the harness | [`docs/PHASEB-IDENTITY-HARNESS.md`](docs/PHASEB-IDENTITY-HARNESS.md) |
| the features, and what they may not read | [`docs/PHASEC-IDENTITY-FEATURES.md`](docs/PHASEC-IDENTITY-FEATURES.md) |
| the fragmentation dose-response | [`docs/PHASED-FRAGMENTATION.md`](docs/PHASED-FRAGMENTATION.md) |
| the case files | [`docs/PHASEE-CASE-FILES.md`](docs/PHASEE-CASE-FILES.md) |
| the refuted prediction | [`docs/negative-results/identity-fragments-worse-refuted.md`](docs/negative-results/identity-fragments-worse-refuted.md) |

## What else was measured on AMLworld
